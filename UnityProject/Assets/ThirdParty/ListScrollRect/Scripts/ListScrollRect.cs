...
public class ListScrollRect : UnityEngine.UI.ScrollRect
{
	...
	public override void OnDrag(PointerEventData eventData)
	{
		...
		{
			//TODO: 不允许上拉
			if (content.rect.height >= 480 && content.rect.height - 480 <= content.anchoredPosition.y + threshold)
			{
				//Debug.Log("上拉刷新...");
			}
			else if (content.rect.height >= 100 && content.anchoredPosition.y < -threshold)
			{
				begain = false;

				//Debug.Log("下拉加载...");
				//sdk本地聊天记录下一页
				SystemEventManager.TriggerEvent(SystemEventName.Drag);
			}
		}
	}
	...
	{
		//Debug.Log($"OnEndDrag pos={content.anchoredPosition.y} rect={content.rect.height}");
		begain = false;

		//float waitTime = 0;
		listInfo.scrollToIndex = -1;
	...
	private void PositionListItem(ListItemInfo listItemInfo, float listPos, bool addedItemAtBeginning)
	{
		...
		SetAnchoredPosition(objRectT, pos);  //重要：默认间距10 //+5导致每次刷新content上移5个单位
